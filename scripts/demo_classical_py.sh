...
# Demo script showing classical load balancer + traffic generator integration using Python
# This script demonstrates how to test the classical load balancer

echo "=== Classical Load Balancer Demo ==="
echo "Testing Classical Load Balancer with Traffic Generator"
echo ""

# Check if Python is available
if ! which python &> /dev/null; then
    echo "Error: Python is required but not installed if you use python 3 run demo_classical_py3.sh"
    exit 1
fi
...
cd ../load_balancer
python c_balancer.py --server localhost 8080 &
LB_PID=$!
cd -
